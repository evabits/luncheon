...
export function SettingsForm({ initialCost }: { initialCost: number }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Cost per lunch (€)</label>
        <input
          type="number"
          value={cost}
      ...
      <button
        type="submit"
        disabled={saving}
        className="w-full py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 disabled:opacity-50 text-sm font-medium"
      >
